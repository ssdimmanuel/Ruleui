...
public class CodeCompletionCore {
	...
	public static class CandidatesCollection {
		/**
		 * Collection of Token ID candidates, each with a follow-on List of subsequent
		 * tokens
		 */
		public Map<Integer, List<Integer>> tokens = new HashMap<>();
		/**
		 * Collection of Rule candidates, each with the callstack of rules to reach the
		 * candidate
		 */
	...
	// A rule which has been visited before with the same input position will always
	// produce the same output positions.
	 ...
	 * position you want to get code completion candidates for). Optionally you can
	 * pass in a parser rule context which limits the ATN walk to only that or
	 * called rules. This can significantly speed up the retrieval process but might
	 * miss some candidates (if they are outside of the given context).
	 */
	public CandidatesCollection collectCandidates(int caretTokenIndex, ParserRuleContext context) {
		...
		for (int ruleId : preferredRules) {
			...
			if (tokens.get(endToken).getType() == Token.EOF) {
				...
				// if last token is not EOF, limit to matching tokens which excludes trailing
				// whitespace
